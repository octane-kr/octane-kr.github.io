...
위 과정에서 $H$의 크기가 애초에 $2k$ 이하이므로 모든 부분집합 $H'$를 따져봐도 $G$에 대한 다항 시간이기는 하지만, 실질적으로 너무 오래 걸리는 것처럼 보인다. 그러나 우리가 확인하고 싶은 조건은 Hall's theorem의 조건과 같으므로 사실 $H$와 $C$ 사이를 이분그래프처럼 보고 최대 매칭을 찾는 문제로 환원 가능하고, 이런 식으로 $H'$을 쉽게 찾을 수 있다.
